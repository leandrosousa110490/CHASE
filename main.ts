...
class FantasyDraftApp {
    ...
    private async createTables(): Promise<void> {
        if (!this.conn) throw new Error('Database connection not established');
    ...
    private generateRandomDraftNumber(): number {
    ...
    private async savePlayer(name: string, draftNumber: number): Promise<void> {
        if (!this.conn) throw new Error('Database connection not established');
    ...
    private async assignDraftNumber(): Promise<void> {
    ...
    private async playerExists(name: string): Promise<boolean> {
        if (!this.conn) return false;

        try {
            const result = await this.conn.query(
                `SELECT COUNT(*) as count FROM draft_players WHERE LOWER(name) = LOWER('${name.replace(/'/g, "''")}')`
            );
    ...
    private async clearAllPlayers(): Promise<void> {
        if (!this.conn) return;
    ...
    private displayPlayersList(players: Player[]): void {
    ...
    private showResult(playerName: string, draftNumber: number): void {
